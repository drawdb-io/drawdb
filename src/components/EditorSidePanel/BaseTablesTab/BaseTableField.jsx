...
import BaseTableFieldDetails from "./BaseTableFieldDetails";

export default function BaseTableField({ data, bid, index }) {
  const { updateBaseTable, baseTables } = useBaseTables();
  ...
  return (
    <div className="hover-1 my-2 flex gap-2 items-center">
      ...
      <div className="min-w-20 flex-1/3">
        <Input
          value={data.name}
          id={`scroll_base_table_${bid}_input_${index}`}
          validateStatus={data.name.trim() === "" ? "error" : "default"}
          readonly={layout.readOnly}
          placeholder={t("name")}
          onChange={(value) => updateField(bid, data.id, { name: value })}
      ...
      <div>
        <Button
          ...
          onClick={() => {
            ...
            setRedoStack([]);
            updateField(bid, data.id, { notNull: !data.notNull });
          }}
        >
          ?
        </Button>
      ...
      <div>
        <Button
          ...
          onClick={() => {
            ...
            setRedoStack([]);
            updateField(bid, data.id, { primary: !data.primary });
          }}
        />
      </div>
        ...
        <Popover
          content={
            <div className="px-1 w-[240px] popover-theme">
              <BaseTableFieldDetails data={data} bid={bid} />
            </div>
          }
          trigger="click"
          position="right"
