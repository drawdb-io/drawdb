...
export default function ControlPanel({
  ...
  const redo = () => {
    ...
    if (a.action === Action.ADD) {
      ...
      if (a.element === ObjectType.AREA) {
        const currentArea = areas.find(ar => ar.id === a.aid);
        undoStateProperties = currentArea ? { ...currentArea } : a.undo;
        updateArea(a.aid, a.redo);
      } else if (a.element === ObjectType.NOTE) {
        const currentNote = notes.find(n => n.id === a.nid);
        undoStateProperties = currentNote ? { ...currentNote } : a.undo;
        updateNote(a.nid, a.redo);
      } else if (a.element === ObjectType.TABLE) {
        const tableBeforeRedo = tables.find(t => t.id === a.tid);
        if (a.component === "field_update") { // Manage for field_update
          if (a.data && typeof a.data.updatedFieldId !== 'undefined' && a.data.appliedValues) {
            actionForUndoStack.data = {
              ...a.data,
              previousFields: tableBeforeRedo ? JSON.parse(JSON.stringify(tableBeforeRedo.fields)) : a.data.fieldsBeforeUndoWasApplied,
            };
            delete actionForUndoStack.data.fieldsBeforeUndoWasApplied;
          ...
          // a is the redo item.
          // a.data.field is the field to delete.
          // a.data.tid is the table ID.
          // The item 'a.data' also contains previousFields, relationship, childFieldsSnapshot
          // which represent the state BEFORE the original deletion. These are for the NEXT UNDO.
          ...
          if (currentTable && currentTable.fields.length > 0) {
            // The last field in the current table is the one that was added.
             actionForUndoStack.data = {
                ...a.data,
                fieldToAdd: JSON.parse(JSON.stringify(currentTable.fields[currentTable.fields.length - 1]))
             };
          ...
          if (currentTable && currentTable.indices.length > 0) {
             actionForUndoStack.data = { ...a.data, indexToAdd: JSON.parse(JSON.stringify(currentTable.indices[currentTable.indices.length - 1])) };
          }
        } else if (a.component === "index") {
          if (currentTable) {
            const currentIndex = currentTable.indices.find(idx => idx.id === a.iid);
          ...
          if (a.data) actionForUndoStack.data = { ...a.data, indexIdToDelete: a.data.id, tid: a.tid };
        } else if (a.component === "self") {
          if (currentTable) redoStateProperties = { redo: { ...currentTable, ...a.redo } };
        }
      } else if (a.element === ObjectType.RELATIONSHIP) {
        ...
        if (a.component === "field_add") {
          if (currentType) {
            redoState = JSON.parse(JSON.stringify(currentType.fields[currentType.fields.length - 1]));
          }
          updateType(a.tid, {
            fields: types.find(ty => ty.id === a.tid)?.fields.filter(
              (_, i, arr) => i !== arr.length - 1,
            ),
          });
        } else if (a.component === "field") {
          if (currentType) {
            const currentField = currentType.fields[a.fid];
            if (currentField) redoStateProperties = { redo: { ...currentField } };
          }
          updateType(a.tid, {
          ...
          if (a.data) redoStateProperties = { fieldIndexToDelete: a.fid, typeId: a.tid };
          setTypes((prev) =>
            prev.map((t, i) => {
